...
const d = document.querySelectorAll(".dd select");
...
const ue = async () => {
  let am = document.querySelector("form input");
  if (!am) {
  ...
  const url = `${nurl}/${fromCurr.value.toLowerCase()}.json`;
  ...
  console.log(rate);
  let f = av * rate;
  msg.innerText = `${av} ${fromCurr.value} = ${f.toFixed(2)} ${toCurr.value}`;
};
...
const uf = (element) => {
  ...
  let sl = `https://flagsapi.com/${code}/flat/64.png`;
